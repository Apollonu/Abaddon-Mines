...
ServerEvents.recipes(event => {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:amadron'}, recipe => {
        ...
        output_variant.includes('pneumaticcraft:gasoline') && output.addProperty('id',
            output_variant.replace('pneumaticcraft:gasoline', unified_individual)
        )
    })
    event.forEachRecipe({type: 'pneumaticcraft:amadron'}, recipe => {
        ...
        if (!output_variant) return
        output_variant.includes('pneumaticcraft:gasoline') && output.addProperty('fluid',
            output_variant.replace('pneumaticcraft:gasoline', unified_individual)
        )
    })
    event.forEachRecipe({type: 'pneumaticcraft:refinery'}, recipe => {
        ...
        output.forEach(entry => {
            ...
            if (!output_variant) return
            if (output_variant === 'pneumaticcraft:gasoline') {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:thermo_plant'}, recipe => {
        ...
        if (!output_variant) return
        if (output_variant.includes('forge:gasoline')) {
            output.remove('tag')
            output.addProperty('fluid',
                output_variant.replace('forge:gasoline', unified_individual)
            )
        }
    })
