...
ServerEvents.recipes(event => {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:amadron'}, recipe => {
        const output = recipe.json.get('output')
        if (!output) return
        const output_variant = output.get('id')?.asString
        if (!output_variant) return
        output_variant.includes('pneumaticcraft:oil') && output.addProperty('id',
            output_variant.replace('pneumaticcraft:oil', unified_individual)
        )
    })
    event.forEachRecipe({type: 'pneumaticcraft:amadron'}, recipe => {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:refinery'}, recipe => {
        ...
        if (!output_variant) return
        if (output_variant.includes('forge:crude_oil')) {
            output.remove('tag')
            output.addProperty('fluid',
                output_variant.replace('forge:crude_oil', unified_individual)
            )
        }
    })
