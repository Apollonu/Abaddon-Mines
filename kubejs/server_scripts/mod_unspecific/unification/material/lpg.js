...
ServerEvents.recipes(event => {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:amadron'}, recipe => {
        ...
        output_variant.includes('pneumaticcraft:lpg') && output.addProperty('id',
            output_variant.replace('pneumaticcraft:lpg', unified_individual)
        )
    })
    event.forEachRecipe({type: 'pneumaticcraft:refinery'}, recipe => {
        ...
        output.forEach(entry => {
            ...
            if (!output_variant) return
            if (output_variant === 'pneumaticcraft:lpg') {
    ...
    event.forEachRecipe({type: 'pneumaticcraft:thermo_plant'}, recipe => {
        ...
        if (!output_variant) return
        output_variant.includes('pneumaticcraft:lpg') && output.addProperty('fluid',
            output_variant.replace('pneumaticcraft:lpg', unified_individual)
        )
    })
    event.forEachRecipe({type: 'pneumaticcraft:thermo_plant'}, recipe => {
        ...
        if (!output_variant) return
        if (output_variant.includes('forge:lpg')) {
            output.remove('tag')
            output.addProperty('fluid',
                output_variant.replace('forge:lpg', unified_individual)
            )
        }
    })
